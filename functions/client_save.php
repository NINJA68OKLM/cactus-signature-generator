...
$i = $_COOKIE['id'];
  ...
  $requete= "UPDATE employes SET nom='".$_COOKIE['nom_'.$i]."', prenom='".$_COOKIE['prenom_'.$i]."', mail='".$_COOKIE['mail_'.$i]."', ld='".$_COOKIE['ld_'.$i]."', fonction='".$_COOKIE['fonction_'.$i]."' WHERE id='".$_COOKIE['idd']."' AND nom='".$_COOKIE['nom_pre_'.$i]."' AND prenom='".$_COOKIE['prenom_pre_'.$i]."'";
  $result = $conn->query($requete);
  echo $requete;
