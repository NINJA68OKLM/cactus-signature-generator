...
$i = $_COOKIE['id'];
  ...
  $requete= "DELETE FROM employes WHERE id='".$_COOKIE['idd']."' AND nom='".$_COOKIE['nom_pre_'.$i]."' AND prenom='".$_COOKIE['prenom_pre_'.$i]."' AND fonction='".$_COOKIE['fonction_pre_'.$i]."'";
  $result = $conn->query($requete);
  echo $requete;
  // 
  $rrequete= $requete= "SELECT * FROM entreprise WHERE id='".$_COOKIE['idd']."'";
  echo $rrequete;
  $result = $conn->query($rrequete);
